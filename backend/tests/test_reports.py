...
def test_create_rejects_bad_audience(client):
    r = client.post(
        "/api/reports",
        json={"period_start": "2026-04-13", "period_end": "2026-04-19", "audience": "nope"},
    )
...
def test_create_rejects_reversed_period(client):
    r = client.post(
        "/api/reports",
        json={"period_start": "2026-04-19", "period_end": "2026-04-13"},
    )
    assert r.status_code == 400
...
def test_create_rejects_bad_date(client):
    r = client.post(
        "/api/reports",
        json={"period_start": "not-a-date", "period_end": "2026-04-13"},
    )
    assert r.status_code == 400
...
def test_patch_changes_period_and_relabels(client):
    created = client.post(
        "/api/reports",
        json={"period_start": "2026-04-13", "period_end": "2026-04-19"},
    ).json()
    rid = created["id"]

    # Shift period to next week — label should auto-update
    r = client.patch(
        f"/api/reports/{rid}",
        json={"period_start": "2026-04-20", "period_end": "2026-04-26"},
    )
    assert r.status_code == 200, r.text
    updated = r.json()
...
def test_get_report_hydrates_cited_evidence(client):
    """GET /reports/{id} should return cited_evidence_detail with full rows, order-preserved."""
    # Seed has a draft report with cited_evidence pointing at seeded evidence.
    report = client.get("/api/reports").json()[0]
    detail = client.get(f"/api/reports/{report['id']}").json()
    ids = detail["cited_evidence"]
    assert len(ids) > 0, "seed should ship a report with citations"
    detail_list = detail["cited_evidence_detail"]
    assert [d["id"] for d in detail_list] == ids  # order preserved
    for d in detail_list:
        assert "text" in d and d["text"]
        assert "category" in d
        assert "event_date" in d
        # thread_title is nullable (evidence without thread) but key exists
        assert "thread_title" in d
        assert isinstance(d["owners"], list)
        assert isinstance(d["tags"], list)


def test_get_report_tombstones_missing_evidence(client, tmp_path):
    """If a cited evidence row is deleted out-of-band, detail still returns a stub."""
    import sqlite3
    # Create a report and push a fake id into cited_evidence.
    created = client.post(
        "/api/reports",
        json={"period_start": "2026-04-13", "period_end": "2026-04-19"},
    ).json()
    # Poke the DB directly — the point is the API should tolerate dangling ids.
    db = sqlite3.connect(os.environ["TRACE_DB_PATH"])
    db.execute(
        'UPDATE report SET cited_evidence_json=? WHERE id=?',
        ('["ev_ghost_123"]', created["id"]),
    )
    db.commit()
    db.close()
