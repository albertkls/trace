...
def test_todo_crud(client):
    ...
    thread_id = create_thread(client, title='待办挂接线程')['id']
    r = client.patch(f"/api/todos/{t['id']}", json={'thread_id': thread_id})
    assert r.status_code == 200
    body = r.json()
    assert body['thread_id'] == thread_id
    assert body['thread_title'] is not None
...
def test_todo_create_validates_thread(client):
    r = client.post('/api/todos', json={'text': '任务', 'thread_id': 'th_nonexistent'})
    assert r.status_code == 404
...
def test_todo_list_ordering(client):
    ...
    c = client.post('/api/todos', json={'text': 'C'}).json()
    d = client.post('/api/todos', json={'text': 'D'}).json()
    client.patch(f"/api/todos/{d['id']}", json={'done': True})
